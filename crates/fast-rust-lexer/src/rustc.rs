...
#![allow(rustc::all)]
#![allow(warnings)]
// Vendored from https://github.com/rust-lang/rust/, commit 4efe3dc40fdd5ed3d001e7736ff384f2d21d7361
...
//! text, and does not report errors, instead storing them as flags on the
//! token.
//!
...
//! be easier.
//!
//! The main entity of this crate is the [`TokenKind`] enum which represents
//! common lexeme types.
//!
...
/// first [`strip_shebang`] and then allow frontmatters with
/// [`FrontmatterAllowed::Yes`].
///
/// When tokenizing a slice of a document, be sure to disallow frontmatters with
...
/// definition. See [Rust language reference](https://doc.rust-lang.org/reference/whitespace.html)
...
/// True if `c` is considered horizontal whitespace according to Rust language
/// definition.
...
/// See [Rust language reference](https://doc.rust-lang.org/reference/identifiers.html) for
/// a formal definition of valid identifier name.
pub fn is_id_continue(c: char) -> bool { unicode_ident::is_xid_continue(c) }
...
impl<'a> Cursor<'a> {
    ...
    pub fn as_str(&self) -> &'a str { self.chars.as_str() }
    ...
    pub(crate) fn is_eof(&self) -> bool { self.chars.as_str().is_empty() }
    ...
    pub(crate) fn bump_bytes(&mut self, n: usize) { self.chars = self.as_str()[n..].chars(); }
    ...
    fn frontmatter(&mut self, has_invalid_preceding_whitespace: bool) -> TokenKind {
        ...
        // copied from `eat_identifier`, but allows `-` and `.` in infostring to allow
        // something like `---Cargo.toml` as a valid opener
        ...
        if let Some(closing) = self.as_str().find(&nl_fence_pattern) {
            ...
            // ---cargo
            // ----
            // combine those stuff into this frontmatter token such that it gets detected
            // later.
            self.eat_until(b'\n');
            found = true;
        }
        ...
        if !found {
            // recovery strategy: a closing statement might have preceding
            // whitespace/newline but not have enough dashes to properly close.
            // In this case, we eat until there, and report a mismatch in the
            // parser.
            let mut rest = self.as_str();
            // We can look for a shorter closing (starting with four dashes but closing with
            // three) and other indications that Rust has started and the
            // infostring has ended.
            let mut potential_closing = rest
                .find("\n---")
                // n.b. only in the case where there are dashes, we move the index to the line where
                // the dashes start as we eat to include that line. For other cases those are Rust
                // code and not included in the frontmatter.
                .map(|x| x + 1)
                .or_else(|| rest.find("\nuse "))
                .or_else(|| rest.find("\n//!"))
                .or_else(|| rest.find("\n#!["));
            ...
            if potential_closing.is_none() {
                // a less fortunate recovery if all else fails which finds any dashes preceded
                // by whitespace on a standalone line. Might be wrong.
    ...
    /// Eats the double-quoted string and returns `n_hashes` and an error if
    /// encountered.
    ...
    fn eat_literal_suffix(&mut self) { self.eat_identifier(); }

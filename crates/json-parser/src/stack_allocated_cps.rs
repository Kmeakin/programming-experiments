...
type Cont<'tokens, 'cont, Arg, Ret> = &'cont mut dyn FnMut((Arg, &'tokens [Token])) -> Ret;
...
fn value<'tokens, T>(
    ...
    match token {
        ...
        Token::LSquare => match rest {
            ...
            _ => value(
                rest,
                &mut |(elem, tokens)| array_loop(vec![elem], tokens, ok, err),
                err,
            ),
        },
        Token::LCurly => match rest {
            ...
            _ => kv_pair(
                rest,
                &mut |(elem, tokens)| object_loop(vec![elem], tokens, ok, err),
                err,
            ),
        },
...
fn array_loop<'tokens, T>(
    ...
    match tokens {
        ...
        [Token::Comma, rest @ ..] => value(
            ...
            &mut |(elem, tokens)| {
                elems.push(elem);
                array_loop(std::mem::take(&mut elems), tokens, ok, err)
            },
            err,
        ),
...
fn kv_pair<'tokens, T>(
    ...
    let (mut key, mut tokens) = match tokens {
    ...
    value(
        tokens,
        &mut |(value, tokens)| ok(((std::mem::take(&mut key), value), tokens)),
        err,
    )
}
...
fn object_loop<'tokens, T>(
    ...
    match tokens {
        ...
        [Token::Comma, rest @ ..] => kv_pair(
            ...
            &mut |(elem, tokens)| {
                elems.push(elem);
                object_loop(std::mem::take(&mut elems), tokens, ok, err)
            },
            err,
        ),
        [_, ..] => err(("unexpected token; expected `]` or `,`", tokens)),

...
pub fn lex_soa(padded_src: &[u8], kinds: &mut Vec<TokenKind>, ends: &mut Vec<u32>) {
    ...
    lex(padded_src, move |kind, _start, end| unsafe {
...
pub fn lex(padded_src: &[u8], mut on_token: impl FnMut(TokenKind, *const u8, *const u8)) {
    unsafe {
        ...
        lex_state.fns[src_start.read() as usize](lex_state, &mut on_token, src_start, src_end);
    }
}

type LexFn<F> = unsafe fn(&LexState<F>, F, *const u8, *const u8) -> ();

struct LexState<F: FnMut(TokenKind, *const u8, *const u8)> {
    fns: [LexFn<F>; 256],
}

impl<F: FnMut(TokenKind, *const u8, *const u8)> LexState<F> {
    pub const fn new() -> Self {
        let fns = const {
            let mut fns: [LexFn<F>; 256] = [Self::unknown; 256];
            let mut i = 0;
            while i < 256 {
                fns[i] = match i as u8 {
    ...
    fn next_state(&self, byte: u8) -> LexFn<F> { self.fns[byte as usize] }

    unsafe fn eof(_: &Self, _on_token: F, _token_start: *const u8, _src_end: *const u8) {}

    unsafe fn unknown(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        let token_end = token_start.add(1);
        on_token(TokenKind::Unknown, token_start, token_end);
        become self.next_state(token_end.read())(self, on_token, token_end, src_end)
    }

    unsafe fn whitespace(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        let token_end = eat_whitespace(token_start.add(1));
        on_token(TokenKind::Whitespace, token_start, token_end);
        become self.next_state(token_end.read())(self, on_token, token_end, src_end)
    }

    unsafe fn punct(&self, mut on_token: F, mut token_start: *const u8, src_end: *const u8) {
        ...
        become self.next_state(token_start.read())(self, on_token, token_start, src_end)
    }

    unsafe fn slash(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        ...
        match token_start.add(1).read() {
            ...
            b'*' => {
                ...
                on_token(TokenKind::BlockComment, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            _ => {
                let token_end = token_start.add(1);
    ...
    unsafe fn alpha(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
    ...
    unsafe fn digit(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
    ...
    unsafe fn quote(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        let token_end = double_quote_string(token_start, src_end);
        on_token(TokenKind::Str, token_start, token_end);
        become self.next_state(token_end.read())(self, on_token, token_end, src_end)
    }

    unsafe fn apostrophe(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        let mut token_end = token_start.add(1);
        if is_ident_start(token_end.read()) {
            ...
            match token_end.read() {
                b'\'' => {
                    token_end = token_end.add(1);
                    on_token(TokenKind::Char, token_start, token_end);
                    become self.next_state(token_end.read())(self, on_token, token_end, src_end)
                }
                _ => {
                    on_token(TokenKind::Lifetime, token_start, token_end);
                    become self.next_state(token_end.read())(self, on_token, token_end, src_end)
                }
            }
        }
    ...
    unsafe fn b(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        match token_start.cast::<[u8; 4]>().read() {
            [b'b', b'\'', ..] => {
                let token_end = single_quote_string(token_start.add(1));
                on_token(TokenKind::BChar, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'b', b'"', ..] => {
                let token_end = double_quote_string(token_start.add(1), src_end);
                on_token(TokenKind::BStr, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'b', b'r', b'"', ..] => {
                let token_end = raw_string(token_start.add(1), src_end);
                on_token(TokenKind::RawBStr, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'b', b'r', b'#', ..] => {
    ...
    unsafe fn c(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        match token_start.cast::<[u8; 4]>().read() {
            [b'c', b'"', ..] => {
                let token_end = double_quote_string(token_start.add(1), src_end);
                on_token(TokenKind::CStr, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'c', b'r', b'"', ..] => {
                let token_end = raw_string(token_start.add(1), src_end);
                on_token(TokenKind::RawCStr, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'c', b'r', b'#', ..] => {
                let token_end = raw_hash_string(token_start.add(1), src_end);
    ...
    unsafe fn r(&self, mut on_token: F, token_start: *const u8, src_end: *const u8) {
        match token_start.cast::<[u8; 4]>().read() {
            [b'r', b'"', ..] => {
                let token_end = raw_string(token_start, src_end);
                on_token(TokenKind::RawStr, token_start, token_end);
                become self.next_state(token_end.read())(self, on_token, token_end, src_end)
            }
            [b'r', b'#', b'#' | b'"', ..] => {
